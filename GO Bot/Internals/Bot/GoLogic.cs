...
namespace GO_Bot.Internals.Bot {

	internal class GoLogic {
		...
		private async Task ExecuteFarmingPokestopsAndPokemons() {
			...
			foreach (var pokeStop in pokeStops) {
				...
				Logger.Write($"Farmed XP: {fortSearch.ExperienceAwarded}, Gems: { fortSearch.GemsAwarded}, Eggs: {fortSearch.PokemonDataEgg} Items: {GetSummedFriendlyNameOfItemAwardList(fortSearch.ItemsAwarded)}", LogLevel.Info);
			...
			do {
			    ...
			    Logger.Write(
			        caughtPokemonResponse.Status == CatchPokemonResponse.Types.CatchStatus.CatchSuccess
			            ? $"We caught a {pokemon.PokemonId} with CP {encounter?.WildPokemon?.PokemonData?.Cp} and CaptureProbability: {encounter?.CaptureProbability.CaptureProbability_.First()} using a {pokeball} in {Math.Round(distance)}m distance"
